...
// should call the forEach() method on the original array to iterate over it
// should return an empty array if receives an empty array as argument
// should return an array with the numbers multiplied by the multiplier
const numbers = [1, 2, 5, 10, 13, 50];

function multiplyBy(someNumbers, multiplier) {
...
console.log(multiplyBy(numbers, 1));
...
// should return the original array if receives an empty array as the second argument
// should correctly filter out the elements of the second array from the first array
...
function filterOut(original, toRemove) {
 if (original.length === 0) {
   ...
   let filteredOut = original.filter(function(animals) {
     return !toRemove.includes(animals);
   });
   return filteredOut;
 }
}
...
function uniquifyArray(someArray) {
  ...
  for (let i = 0; i < someArray.length; i++) {
    if (uniqified.indexOf(someArray[i]) === -1) {
      uniqified.push(someArray[i]);
    }
  }
  return uniqified;
}
